...
2. **한국어 주석** — 모든 docstring과 inline comment는 한국어로 작성.
   - 함수: docstring 한국어 + Args/Returns/Raises 명시 (필수)
   - 모듈: 작성 의도 + 담당자 명시 (필수)
   - 매직 넘버: `# 근거 한 줄` (권장 — 예: `TF_WEIGHTS = {1H: 2, 1W: 25}  # 1W 가 장기 추세 더 무겁게`)
   - 복잡 로직 분기: `# Why: 의도` (권장 — 다른 방식 대비 이 방식 선택 이유)
   - 잘 알려진 패턴 (for, list comp 등): 주석 X (불필요)
   - **목적**: WooJae 같은 점검 역할이 코드 의도 빠르게 이해 + 매직 넘버 근거 추적
